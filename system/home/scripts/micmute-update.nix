...
pkgs.writeShellScriptBin "micmute-update" ''
  ...
  if [ -z "''${line:-}" ] && command -v ${pactl} >/dev/null 2>&1; then
    mute_word="$(${pactl} get-source-mute @DEFAULT_SOURCE@ 2>/dev/null | ${awk} '{print tolower($2)}' || true)"
    if [ "$mute_word" = "yes" ]; then
      is_muted=true
    fi
    vol_percent="$(${pactl} get-source-volume @DEFAULT_SOURCE@ 2>/dev/null | grep -oE '[0-9]+%' | head -n1 | tr -d '%' || echo 0)"
